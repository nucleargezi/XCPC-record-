...
NAME MeIoN_is_UMP45() {
    ...
    se.emplace('W', 'E'), se.emplace('W', 'Y');
    se.emplace('J', 'W'), se.emplace('J', 'H');
    se.emplace('E', 'J'), se.emplace('E', 'Y');
    se.emplace('Y', 'J'), se.emplace('Y', 'H');
    se.emplace('H', 'W'), se.emplace('H', 'E');
    std::cin >> n >> m;
    vector<ch> kda(n), kdb(n);
    vector<int> la(n), lb(n);
    int ca(0), cb(0);
    for (meion &i : kda)
        std::cin >> str, i = str[0], ca += i == 'Y';
    for (meion &i : kdb)
        std::cin >> str, i = str[0], cb += i == 'Y';
    std::cin >> la >> lb;
    for (int i = 0; i < n; ++i) {
        if (kda[i] == 'J') la[i] += ca;
    }
    ...
    for (int i = 0; i < n; ++i) for (int k = 0; k < n; ++k) {
        if (se.contains({kda[i], kdb[k]})) 
            FL::add(i + 1, k + 1 + n, 1);
    }
    std::cout << std::min(m, (int)FL::sap()) << '\n';
}
// 日々を貪り尽くしてきた
int main() {
#ifdef guidingstar
