...
NAME MeIoN_is_UMP45() {
    int n, m, d;
    std::cin >> n >> m >> d;
    int dx[] = {d, -d, 0, 0}, dy[] = {0, 0, d, -d};
    int s = 0, t = n * m << 1;
    FL::III(s, t, t + 1);

#define id(x, y) ((x - 1) * m + y)
#define ck(x, y) (x > 0 and x < n + 1 and y > 0 and y < m + 1)

    for (int i = 1; i < n + 1; ++i) {
        for (int k = 1; k < m + 1; ++k) {
            static char c; std::cin >> c;
            mp[i][k] = c - '0';
        }
    }
    for (int i = 1; i < n + 1; ++i) {
        for (int k = 1; k < m + 1; ++k) {
            if (not mp[i][k]) continue;
            FL::add(id(i, k), id(i, k) + n * m, mp[i][k]);
            int out = 0;
            for (int p = 0; p < 4; ++p) {
            ...
            if (out) 
                FL::add(id(i, k) + n * m, t, mp[i][k]);
            for (int x = std::max(1, i - d); x <= std::min(n, i + d); ++x) {
                for (int y = std::max(1, k - d); y <= std::min(m, k + d); ++y) {
                    if (ksm(std::abs(i - x), 2) + ksm(std::abs(k - y), 2) > d * d) continue;
                    if (mp[x][y] and id(i, k) != id(x, y)) 
                        FL::add(id(i, k) + n * m, id(x, y), std::min(mp[i][k], mp[x][y]));
                }
            }
        }
    }
