...
namespace MeIoN_Pre_Things {
    ...
    template <typename T> int MeIoN_count(T n) { iroha std::__popcount(n); }
    template <typename T> int MeIoN_clz(T n)   { iroha std::__countl_zero(n); }
    template <typename T> T lowbit(T x) { iroha x & -x; }
    template <typename T> void MAX(T& a, T b) { a = std::max(a, b); }
    template <typename T> void MIN(T& a, T b) { a = std::min(a, b); }
    template <typename T> void unique (vector<T> &v) { MEION::sort(v); v.erase(std::unique(v.begin(), v.end()), v.end()); v.shrink_to_fit(); }
...
namespace MeIoN_IO {
    ...
    void U() {} template <typename... Args> void U(Args&&... any) { ((std::cout << any), ...); }
    void UU() { std::cout << " "; } template <typename... Args> void UU(Args&&... any) { ((std::cout << any << " "), ...); }
    void UL() { std::cout << "\n"; } template <typename... Args> void UL(Args&&... any) { ((std::cout << any << "\n"), ...); }
    template <typename T, typename S> void Uif(bool ok, T a, S b) { std::cout << (ok ? a : b); }
    void YES(bool ok) { UL(ok ? "YES" : "NO"); } void Yes(bool ok) { UL(ok ? "Yes" : "No"); } void yes(bool ok) { UL(ok ? "yes" : "no"); }
    void NO(bool ok)  { UL(ok ? "NO" : "YES"); } void No(bool ok)  { UL(ok ? "No" : "Yes"); } void no(bool ok)  { UL(ok ? "no" : "yes"); }
    void ALICE(bool ok) { UL(ok ? "ALICE" : "BOB"); } void Alice(bool ok) { UL(ok ? "Alice" : "Bob"); } void alice(bool ok) { UL(ok ? "alice" : "bob"); }
    void BOB(bool ok) { UL(ok ? "BOB" : "ALICE"); } void Bob(bool ok) { UL(ok ? "Bob" : "Alice"); } void bob(bool ok) { UL(ok ? "bob" : "alice"); }
    void POSSIBLE(bool ok) { UL(ok ? "POSSIBLE" : "IMPOSSIBLE"); } void Possible(bool ok) { UL(ok ? "Possible" : "Impossible"); } void possible(bool ok) { UL(ok ? "possible" : "impossible"); }
    void IMPOSSIBLE(bool ok) { UL(not ok ? "POSSIBLE" : "IMPOSSIBLE"); } void Impossible(bool ok) { UL(not ok ? "Possible" : "Impossible"); } void impossible(bool ok) { UL(not ok ? "possible" : "impossible"); }
...
NAME MeIoN_is_UMP45() {
    ...
    for (int i = 0; i < n - 1; ++i) {
        const meion [x, y] = a[i];
        const meion [xx, yy] = a[i + 1];
        const int point = m + i + 1;
        FL::add(s, point, 1);
        ld dis = std::sqrt(ksm(std::abs(x - xx), 2) + ksm(std::abs(y - yy), 2) << 2);
        for (int p = 1; const meion [xxx, yyy] : b) {
            ld dd = std::sqrt(ksm(std::abs(x - xxx), 2) + ksm(std::abs(y - yyy), 2)) + 
                    std::sqrt(ksm(std::abs(xx - xxx), 2) + ksm(std::abs(yy - yyy), 2));
            if (dd <= dis + eps) {
                FL::add(point, p, 1);
            }
        ++p; }
    ...
    int sz = FL::sap() + n;
    vector<pair<int, int>> dog, ans;
    std::cout << sz << '\n';
    for (int i = 2; i < FL::tot + 1; i += 2) {
        const meion [x, y] = FL::e[i];
        if (x == s or y == t or FL::f[i]) continue;
        dog.emplace_back(x - m, y - 1);
    }
    MEION::reverse(dog);
    for (int i = 0; i < n; ++i) {
        if (not dog.empty() and dog.back().first == i) {
            ans.push_back(b[dog.back().second]);
            dog.pop_back();
        }
        ans.push_back(a[i]);
    }
