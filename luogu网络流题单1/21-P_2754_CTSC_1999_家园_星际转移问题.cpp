...
NAME MeIoN_is_UMP45() {
    ...
    for (meion &[sz, len, road] : v) {
        ...
        for (int i = 0; i < len - 1; ++i) {
            dsu.merge(road[i], road[i + 1]);
        }
    }
    if (not dsu.eq(0, n + 1)) iroha std::cout << "0\n", void();

#define id(x, y) ((x) * (n + 2) + y + 1)

    int l = 1, r = n * n << 4, ans = r;
    while (l <= r) {
        int mid = l + r >> 1;
        int s = 0, t = (n + 2) * (mid + 1) + 1;
        FL::III(s, t, t + 1);
        FL::add(s, id(0, 0), k);
        FL::add(id(mid, n + 1), t, FL::inf);
        for (int i = 0; i < mid; ++i) {
            for (int k = 0; k < n + 2; ++k) {
                FL::add(id(i, k), id(i + 1, k), FL::inf);
            }
        }
        for (const meion &[sz, len, road] : v) {
            for (int i = 0; i < mid; ++i) {
                FL::add(id(i, road[i % len]), id(i + 1, road[(i + 1) % len]), sz);
            }
        }
        int res = FL::sap();
        if (res >= k) {
