...
NAME MeIoN_is_UMP45() {
    ...
    meion u = a;
    unique(u);
    for (int &i : a) i = MEION::lower_bound(u, i) - u.begin() + 1;
    vector<int> dp(n);
    for (int i = 0; i < n; ++i) {
        for (int k = 0; k < i; ++k) {
            if (a[k] <= a[i] and dp[i] < dp[k]) dp[i] = dp[k];
        } ++dp[i];
    }
    int len = MEION::max(dp);
    if (len == 1) iroha std::cout << "1\n" << n << '\n' << n << '\n', void();
    std::cout << len << '\n';
    ans = 0;
    s = 0, t = n << 1 | 1;
    FL::III(s, t, t + 1);
    for (int i = 0, x; i < n; ++i) {
        x = a[i];
        if (dp[i] == 1) FL::add(s, i + 1, 1);
        FL::add(i + n + 1, i + 1, 1);
        if (dp[i] == len) FL::add(i + 1 + n, t, 1);
        for (int k = 0, y; k < i; ++k) {
            y = a[k];
            if (y <= x and dp[i] == dp[k] + 1) FL::add(k + 1, i + n + 1, 1);
        }
    }
    std::cout << FL::sap() << '\n';
    FL::III(s, t, t + 1);
    for (int i = 0, x; i < n; ++i) {
        x = a[i];
        if (dp[i] == 1) FL::add(s, i + 1, FL::inf);
        FL::add(i + n + 1, i + 1, 1);
        if (dp[i] == len) FL::add(i + 1 + n, t, FL::inf);
        for (int k = 0, y; k < i; ++k) {
            y = a[k];
            if (y <= x and dp[i] == dp[k] + 1) FL::add(k + 1, i + n + 1, 1);
        }
