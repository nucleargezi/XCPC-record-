...
#define ck(x, y) (x > 0 and x < n + 1 and y > 0 and y < m + 1)
    ...
    for (int i = 1; i < n + 1; ++i) {
        for (int k = 1; k < m + 1; ++k) {
            std::cin >> mp[i][k];
            ans += mp[i][k];
            if ((i + k) & 1) FL::add(s, id(i, k), mp[i][k]);
            else FL::add(id(i, k), t, mp[i][k]);
        }
    }
    for (int i = 1; i < n + 1; ++i) {
        for (int k = 1; k < m + 1; ++k) {
            if ((i + k) & 1) for (const meion [dx, dy] : {P{1, 0}, P{-1, 0}, P{0, 1}, P{0, -1}}) {
                int x = i + dx, y = k + dy;
                if (ck(x, y)) {
                    FL::add(id(i, k), id(x, y), FL::inf);
                }
            }
        }
    }
