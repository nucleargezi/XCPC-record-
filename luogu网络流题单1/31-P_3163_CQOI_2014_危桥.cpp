...
NAME MeIoN_is_UMP45() {
    int n, s, t, ss, tt, ati, bti;
    while (std::cin >> n >> s >> t >> ati >> ss >> tt >> bti) {
        ++s, ++t, ++ss, ++tt;
        int S = 0, T = n + 1;
        int ok = 1;
        FL::III(S, T, T + 1);
        static array<array<int, 0721>, 0721> mp;
        for (int i = 1; i < n + 1; ++i) {
            for (int k = 1; k < n + 1; ++k) {
                static char ch; std::cin >> ch, mp[i][k] = ch;
                if (ch == 'O')
                    FL::add(i, k, 1);
                else if (ch == 'N')
                    FL::add(i, k, FL::inf);
        ...
        FL::add(S, s, ati), FL::add(S, ss, bti);
        FL::add(t, T, ati), FL::add(tt, T, bti);
        ok &= (FL::sap() == ati + bti);
        std::swap(ss, tt);
        FL::III(S, T, T + 1);
        for (int i = 1; i < n + 1; ++i) {
            for (int k = 1; k < n + 1; ++k) {
                static char ch; ch = mp[i][k];
                if (ch == 'O')
                    FL::add(i, k, 1);
                else if (ch == 'N')
                    FL::add(i, k, FL::inf);
