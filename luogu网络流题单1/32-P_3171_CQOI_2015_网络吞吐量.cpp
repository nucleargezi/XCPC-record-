...
NAME MeIoN_is_UMP45() {
    ...
    for (int i = 1; i < n + 1; ++i) {
        if (i > 1 and i < n) FL::add(i, i + n, ci[i - 1]);
        else FL::add(i, i + n, FL::inf);
    }
    FL::add(s, 1, FL::inf), FL::add(n << 1, t, FL::inf);
    std::cout << FL::sap() << '\n';
}
// 日々を貪り尽くしてきた
int main() {
