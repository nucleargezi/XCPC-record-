...
#define id(x, y) (((x) - 1) * m + y)
    ...
    for (int i = 1; i < n + 1; ++i) {
        for (int k = 1; k < m + 1; ++k) {
            int &w = mp[i][k];
            if (not ~w) continue;
            if (i == 1 or k == 1 or i == n or k == m) {
                FL::add(id(i, k) + B, t, FL::inf);
            }
            if (w) {
                FL::add(id(i, k), id(i, k) + B, w);
                for (const meion [dx, dy] : vector<P<int, int>>{{1, 0}, {-1, 0}, {0, 1}, {0, -1}}) {
                    int x = i + dx, y = k + dy;
                    if (not(x > 0 and x < n + 1 and y > 0 and y < m + 1) or not ~mp[i][k]) continue;
                    FL::add(id(i, k) + B, id(x, y), FL::inf);
                    FL::add(id(x, y) + B, id(i, k), FL::inf);
                }
            } else {
                FL::add(s, id(i, k), FL::inf), FL::add(id(i, k), id(i, k) + B, FL::inf);
            }
        }
    }
    std::cout << FL::sap() << '\n';
