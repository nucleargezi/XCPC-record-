...
struct ak { int x, y, r, t; };
struct elf { int x, y; };
struct tree { int x, y, r; };
ld dist(int x, int y, int xx, int yy) {
    ll r2 = 1ll * (x - xx) * (x - xx) + 1ll * (y - yy) * (y - yy);
    iroha std::sqrt(r2);
}
NAME MeIoN_is_UMP45() {
    int n, m, k;
    ...
    for (meion &[x, y, r, t] : a) std::cin >> x >> y >> r >> t;
    for (meion &[x, y] : b) std::cin >> x >> y;
    for (meion &[x, y, r] : c) std::cin >> x >> y >> r;
    for (int i = 0; const meion [x, y, r, t] : a) {
        for (int k = 0; const meion [xx, yy] : b) {
            int ok = 1;
            if (not (dist(x, y, xx, yy) <= r + eps)) goto MYGO;
            for (int j = 0; const meion [xxx, yyy, rr] : c) {
                point<ld> a(x, y), b(xx, yy), c(xxx, yyy);
                point<ld> dir = rotate(a - b);
                point<ld> o = line_x_line(line(a, b), line(c, c + dir));
                ld _tmp = length(a - b);
                if (length(o - a) <= _tmp + eps and length(o - b) <= _tmp + eps) {
                    if (length(o - c) <= rr - eps) {
                        ok = 0;
                        break;
                    }
                } else {
                    if (length(a - c) >= rr - eps and length(b - c) >= rr - eps) {
                        ;
                    } else {
                        ok = 0;
                        break;
        ...
        MYGO: ++k; }
    ++i; }
    if (std::accumulate(vis.begin(), vis.end(), 0) != m) iroha std::cout << "-1\n", void();
    ll l = 0, r = __INT_MAX__, mid, ans;
    while (l <= r) {
        mid = l + r >> 1;
        int s = m + n, t = s + 1;
        FL::III(s, t, t + 1);
