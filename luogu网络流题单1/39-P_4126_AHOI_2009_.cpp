...
NAME MeIoN_is_UMP45() {
    ...
    for (int i = 0, l, r, w; i < m; ++i) {
        std::cin >> l >> r >> w;
        FL::add(l, r, w);
        v[l].emplace_back(r, i + 1 << 1);
        v[r].emplace_back(l, i + 1 << 1 | 1);
    }
    FL::sap();
    vector<int> dfn(n + 1), low(n + 1), sz(n + 1), id(n + 1), st;
    vector<bool> vis(n + 1);
    int cnt = 0, tot = 0;
    meion tj = [&] (meion &&se, int n)->void {
        dfn[n] = low[n] = ++tot;
        vis[n] = 1;
        st.emplace_back(n);
        for (const meion [i, id] : v[n]) {
            if (not FL::f[id]) continue;
            if (not dfn[i]) {
                se(se, i);
    ...
    for (int i = 1; i < n + 1; ++i) 
        if (not dfn[i]) tj(tj, i), st.clear();
    for (int i = 2; i < FL::tot + 1; i += 2) {
        const meion [l, r] = FL::e[i];
        if (not FL::f[i] and id[l] != id[r]) {
            std::cout << "1 " << (id[l] == id[s] and id[r] == id[t]) << '\n';
        } else {
            std::cout << "0 0\n";
        }
    }
