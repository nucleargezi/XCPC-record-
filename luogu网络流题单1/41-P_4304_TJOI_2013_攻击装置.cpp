...
NAME MeIoN_is_UMP45() {
    ...
    int s = 0, t = n * n + 1;
    FL::III(s, t, t + 1);
#define id(x, y) (x * n - n + y)
#define ck(x, y) (x > 0 and x <= n and y > 0 and y <= n)
    for (int i = 1; i < n + 1; ++i) {
        for (int k = 1, x, y; k < n + 1; ++k) {
            if (i + k & 1) 
                FL::add(s, id(i, k), 1);
            else 
                FL::add(id(i, k), t, 1);
            if (mp[i][k] or not(i + k & 1)) continue;
            for (int p = 0; p < 8; ++p) {
                x = i + dx[p], y = k + dy[p];
                if (not ck(x, y) or mp[x][y]) continue;
                FL::add(id(i, k), id(x, y), 1);
            }
        }
    }
    std::cout << n * n - FL::sap() - c << '\n';
#undef ck
#undef id
}
// 日々を貪り尽くしてきた
