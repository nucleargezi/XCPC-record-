...
#define ck(x, y) (x > 0 and x < n + 1 and y > 0 and y < m + 1)
    ...
    for (int i = 1; i < n + 1; ++i) {
        for (int k = 1; k < m + 1; ++k) {
            int sam_a; std::cin >> sam_a;
            all += sam_a;
            FL::add(s, id(i, k) + n * m, sam_a);
            FL::add(id(i, k) + n * m, id(i, k), FL::inf);
            for (const meion [dx, dy] : vector<pair<int, int>>{{1, 0}, {-1, 0}, {0, 1}, {0, -1}}) {
                int x = dx + i, y = dy + k;
                if (not ck(x, y)) continue;
                FL::add(id(i, k) + n * m, id(x, y), FL::inf);
            }
        }
    }
    for (int i = 1; i < n + 1; ++i) {
        for (int k = 1; k < m + 1; ++k) {
            int sam_s; std::cin >> sam_s;
            all += sam_s;
            FL::add(id(i, k) + n * m * 2, t, sam_s);
            FL::add(id(i, k), id(i, k) + n * m * 2, FL::inf);
            for (const meion [dx, dy] : vector<pair<int, int>>{{1, 0}, {-1, 0}, {0, 1}, {0, -1}}) {
                int x = dx + i, y = dy + k;
                if (not ck(x, y)) continue;
                FL::add(id(x, y), id(i, k) + n * m * 2, FL::inf);
            }
        }
    }
    std::cout << all - FL::sap() << '\n';
