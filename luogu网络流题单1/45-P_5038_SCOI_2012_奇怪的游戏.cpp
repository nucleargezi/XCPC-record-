...
NAME MeIoN_is_UMP45() {
    int n, m;
    std::cin >> n >> m;
    array<array<ll, 0721>, 0721> mp;
    ll l = 0, r = INT >> 1;
    int pl = 0, pr = 0;
    ll sl = 0, sr = 0;
    for (int i = 1; i < n + 1; ++i) {
        for (int k = 1, x; k < m + 1; ++k) {
            std::cin >> mp[i][k];
            if ((i + k) & 1) pl++, sl += mp[i][k];
            else pr++, sr += mp[i][k];
            l = std::max(l, mp[i][k]);
        }
    }
...
#define ok(x, y) (x > 0 and x < n + 1 and y > 0 and y < m + 1)
    ...
    if (pl ^ pr) {
        ...
        if (sl ^ sr) ans = -1;
            ...
            while (l <= r) {
                ll mid = l + r >> 1;
                if (ck(mid)) r = mid - 1, ans = mid * pl - sl;
                else l = mid + 1;
            }
        }
    }
